        ...
        <div class="flex xl:flex-row flex-col gap-2.5">
            <div class="panel px-0 flex-1 py-6 ltr:xl:mr-6 rtl:xl:ml-6">
                <div class="flex justify-between flex-wrap px-4">
                    <div class="text-lg font-semibold ltr:sm:text-left rtl:sm:text-right text-center">{{ __('Add New Coin') }}</div>
                </div>
                <hr class="border-[#e0e6ed] dark:border-[#1b2e4b] my-6">
                <form method="POST" action="{{ route('adminCustomPageSave') }}" enctype="multipart/form-data">
                 @csrf
                 @if(isset($item)) <input type="text" name="edit_id" value="{{ $item->id }}" /> @endif
                    <div class="mt-8 px-4">
                        <div class="text-lg font-semibold">{{ __('Coin Info') }}</div>
                        <div class="flex justify-between lg:flex-row flex-col">
                            <div class="lg:w-2/2 w-full ltr:lg:mr-6 rtl:lg:ml-6 mb-6">
                                
                                <div class="items-center mt-4">
                                    <label for="status" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Page Title') }} <span class="text-danger">*</span></label>
                                    <input id="question" type="text" name="title" class="form-input flex-1"
                                        @if(isset($item)) value="{{ $item->title }}" @else value="{{ old('title') }}" @endif />
                                </div>

                                <div class="mt-4 items-center">
                                    <label for="question" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Page Body') }} <span class="text-danger">*</span></label>
                                    <textarea name="body" id="summernote" class="ltr:mr-2 rtl:ml-2 w-3/3 mb-0"
                                        >{{ (isset($item)) ? $item->body : old('body') }}</textarea>
                                </div>
    ...
    <script>
        ...
        function add_coin_ui_change(){ // name="coin_type"
            ...
            if(currency == 1){
                ...
                coin_type_select.classList.add("hidden");
                coin_type_input.classList.remove("hidden");
                $("#coin_api").show()
                $("#coin_rate_api").show()
            }
            ...
            if(currency == 2){
                ...
                coin_type_select.classList.remove("hidden");
                coin_type_input.classList.add("hidden");
