    ...
    <div >
        <div class="flex xl:flex-row flex-col gap-2.5">
            <div class="panel px-0 flex-1 py-6 ltr:xl:mr-6 rtl:xl:ml-6">
                ...
                <form method="POST" action="{{ route('roleStoreProcess') }}" enctype="multipart/form-data">
                 @csrf
                <div class="mt-8 px-4">
                    <div class="flex justify-between lg:flex-row flex-col">
                        <div class="lg:w-1/2 w-full ltr:lg:mr-6 rtl:lg:ml-6 mb-6">
                            <div class="text-lg font-semibold">{{ __('Role Info') }}</div>
                            <div class="mt-4 flex items-center">
                                <label for="question" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Role Title') }} <span class="text-danger">*</span></label>
                        ...
                        <div class="lg:w-1/2 w-full">
                            <div class="text-lg font-semibold">{{ __('Role Action') }}</div>
                            @foreach (roleActionArray() as $val)
                                <div class="flex items-center mt-4">
                                    <input @if(isset($item)) @if(in_array($val['code'],explode('|',$item->actions))) checked @endif @endif type="checkbox" name="actions[]" value="{{ $val['code'] }}" class="form-checkbox"> {{ $val['name'] }}
                                </div>
                            @endforeach
