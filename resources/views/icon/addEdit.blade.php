        ...
        <div class="flex xl:flex-row flex-col gap-2.5">
            <div class="panel px-0 flex-1 py-6 ltr:xl:mr-6 rtl:xl:ml-6">
                <div class="flex justify-between flex-wrap px-4">
                    <div class="text-lg font-semibold ltr:sm:text-left rtl:sm:text-right text-center">{{ __('Add New Category') }}</div>
                </div>
                    ...
                    <div class="flex justify-between lg:flex-row flex-col">
                        <div class="lg:w-1/2 w-full ltr:lg:mr-6 rtl:lg:ml-6 mb-6">
                            ...
                            <div class="mt-4 items-center">
                                <label for="question" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Title') }} <span class="text-danger">*</span></label>
                            ...
                            <div class="items-center mt-4">
                                <label for="status" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Category') }} <span class="text-danger">*</span></label>
                                <select id="currencySelect" name="category_id" class="form-select flex-1">
                                    @foreach($categories as $category)
                                        <option @if(isset($item) && $item->category_id == $category->id) selected @endif value="{{ $category->id }}"> {{ $category->title }} </option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="grid grid-cols-1 mt-1 sm:grid-cols-2 gap-4">
                                <input type="file" name="icon" class="mt-4 dropify" data-default-file="{{ isset($item->icon) ? asset(IMG_PATH.($item->icon ?? '')) : '' }}" />
                            </div>
