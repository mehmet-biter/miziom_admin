<x-layout.default>
        ...
        <div class="flex xl:flex-row flex-col gap-2.5">
            <div class="panel px-0 flex-1 py-6 ltr:xl:mr-6 rtl:xl:ml-6">
                <div class="flex justify-between flex-wrap px-4">
                    <div class="text-lg font-semibold ltr:sm:text-left rtl:sm:text-right text-center">{{ __('Update New Coin') }}</div>
                </div>
                ...
                <div class="mt-8 px-4">
                    <div class="flex justify-between lg:flex-col flex-col">
                        <div class="w-full ltr:lg:mr-6 rtl:lg:ml-6 mb-6">
                            <div class="text-lg font-semibold">{{ __('Coin Info') }}</div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class=" mt-4">
                                    <label for="name" class="ltr:mr-2 rtl:ml-2 w-3/3 mb-0">{{ __('Coin Name') }} </label>
                                ...
                                <div class=" mt-4">
                                    ...
                                    <input id="name" type="text" name="coin_type" class="form-input flex-1"
                                    ...
                                    <div class=" mt-4">
                                        ...
                                        <select id="status" name="network" class="form-select flex-1">
                                            @foreach(api_settings() as $key => $value)
                                                <option @if(isset($item) && $item->network == $key) selected @endif value="{{$key}}">{{$value}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="mt-4 ">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="mt-4 ">
                                    <label for="question" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Minimum Withdrawal') }} </label>
                                    <input id="question" type="text" name="minimum_withdrawal" class="form-input flex-1"
                                        @if(isset($item)) value="{{ $item->minimum_withdrawal }}" @else value="{{ old('minimum_withdrawal') }}" @endif />
                                </div>
                                
                                <div class="mt-4 ">
                                    <label for="question" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Maximum Withdrawal') }} </label>
                                    <input id="question" type="text" name="maximum_withdrawal" class="form-input flex-1"
                                        @if(isset($item)) value="{{ $item->maximum_withdrawal }}" @else value="{{ old('maximum_withdrawal') }}" @endif />
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class=" mt-4">
                                    <label for="status" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Withdrawal Fees Type') }} <span class="text-danger">*</span></label>
                                    <select id="status" name="withdrawal_fees_type" class="form-select flex-1">
                                        @foreach(discount_type() as $key => $value)
                                            <option @if(isset($item) && $item->withdrawal_fees_type == $key) selected @endif value="{{$key}}">{{$value}}</option>
                                        @endforeach
                                    </select>
                                </div>

                                <div class="mt-4 ">
                                    <label for="question" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Withdrawal Fees') }} </label>
                                    <input id="question" type="text" name="withdrawal_fees" class="form-input flex-1"
                                        @if(isset($item)) value="{{ $item->withdrawal_fees }}" @else value="{{ old('withdrawal_fees') }}" @endif />
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div class=" mt-4">
                                    ...
                                    <select id="status" name="admin_approval" class="form-select flex-1">
                            ...
                            <div class="flex">
                                <div class="mt-4 ">
                                    <label for="question" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Deposit Status') }}</label>
                                    <label class="w-12 h-6 relative">
                                        <input @if(isset($item) && $item?->is_deposit) checked @endif name="is_deposit" type="checkbox" class="custom_switch absolute w-full h-full opacity-0 z-10 cursor-pointer peer" id="custom_switch_checkbox4" />
                                ...
                                <div class="mt-4 ">
                                    <label for="question" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Withdrawal Status') }}</label>
                                    <label class="w-12 h-6 relative">
                                        <input @if(isset($item) && $item?->is_withdrawal) checked @endif name="is_withdrawal" type="checkbox" class="custom_switch absolute w-full h-full opacity-0 z-10 cursor-pointer peer" id="custom_switch_checkbox4" />
                                        <span for="custom_switch_checkbox4" class="bg-[#ebedf2] dark:bg-dark block h-full rounded-full before:absolute before:left-1 before:bg-white dark:before:bg-white-dark dark:peer-checked:before:bg-white before:bottom-1 before:w-4 before:h-4 before:rounded-full peer-checked:before:left-7 peer-checked:bg-blue-500 before:transition-all before:duration-300"></span>
                                    </label>                              
                                </div>
                                ...
                                <div class="mt-4 ">
                                    <label for="question" class="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">{{ __('Active Status') }}</label>
                                    <label class="w-12 h-6 relative">
                            ...
                            <div class="custom-file-container w-1/2" data-upload-id="coinIconUpload"></div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function(e) {
            new FileUploadWithPreview.FileUploadWithPreview('coinIconUpload', {
                Options:{
                    showDeleteButtonOnImages: true
                },
                images: {
                    baseImage: '{{ isset($item) ? $item->coin_icon : '' }}',
                    backgroundImage: '',
                },
                text:{
                    label: '{{ __("Upload Icon") }}',
                }
            });
        });
    </script>
